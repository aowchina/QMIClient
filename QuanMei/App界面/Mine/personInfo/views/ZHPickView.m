...
#define SLLeftMagin 0.f
#define LineColor [UIColor colorWithRed:251.f/255 green:61.f/255 blue:123.f/255 alpha:1]
...
-(void)setUpPickView{
    ...
    pickView.frame=CGRectMake(0, ZHToobarHeight, [UIScreen mainScreen].bounds.size.width, pickView.frame.size.height * 2 / 3);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.frame=CGRectMake(SLLeftMagin, ZHToobarHeight, [UIScreen mainScreen].bounds.size.width - SLLeftMagin * 2, datePicker.frame.size.height);
...
- (void)buildLine {
    UILabel *linelabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, _toolbar.frame.size.width, 1)];
    linelabel.backgroundColor = LineColor;
    [self addSubview:linelabel];
    UILabel *lineLabelBottom = [[UILabel alloc] initWithFrame:CGRectMake(0, _toolbar.frame.size.height - 1, _toolbar.frame.size.width, 1)];
...
- (void)buildLabel {
    self.titleLabel = [[UILabel alloc] initWithFrame:
                       CGRectMake(0, ZHToobarHeight + _pickeviewHeight, [UIScreen mainScreen].bounds.size.width, 40)];
    self.titleLabel.backgroundColor = [UIColor whiteColor];
    self.titleLabel.textAlignment = NSTextAlignmentLeft;
    
    UIColor *lineColor = [UIColor colorWithRed:251.f/255 green:61.f/255 blue:123.f/255 alpha:1];
    
    self.titleLabel.textColor = lineColor;
    self.titleLabel.text = @"正在选择生日";
    [self addSubview:self.titleLabel];
    self.titleLabel.backgroundColor = [UIColor whiteColor];
    
    UILabel *lineLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, _titleLabel.frame.origin.y + 1, self.titleLabel.frame.size.width, 1)];
    
    lineLabel.backgroundColor = lineColor;
    [self addSubview:lineLabel];
//    [self insertSubview:lineLabel atIndex:1000];
    UILabel *lineLabel1 = [[UILabel alloc]initWithFrame:CGRectMake(0, _titleLabel.frame.origin.y + _titleLabel.frame.size.height - 1, self.titleLabel.frame.size.width, 1)];
    
    lineLabel1.backgroundColor = lineColor;
    [self addSubview:lineLabel1];
//    [self insertSubview:lineLabel1 atIndex:999];
...
#pragma mark - action
- (void)handleCancel:(UIBarButtonItem *)item {
    if (self.delegate && [self.delegate respondsToSelector:@selector(ZHPickView:cancelBtnDidClick:)]) {
        [self.delegate ZHPickView:self cancelBtnDidClick:item];
    }
}
        ...
        if (_resultString) {
            ...
            if (_isLevelString) {
              ...
              _resultString=[NSString stringWithFormat:@"%@-%@",_state,_city];
                [SLValue_Singleton shareInstance].province = [NSString stringWithFormat:@"%@",_state];
                [SLValue_Singleton shareInstance].city = [NSString stringWithFormat:@"%@",_city];
           }
        }
    }else if (_datePicker) {
